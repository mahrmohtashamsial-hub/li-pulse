...
export const actorRegistry: Record<ActorKey, ActorDefinition> = {
  posts: {
    key: "posts", label: "Posts & reposts", actorId: "apimaestro~linkedin-batch-profile-posts-scraper", costPerResultUsd: 0.005,
  ...
  comments: {
    key: "comments", label: "Comments given", actorId: "apimaestro~linkedin-profile-comments", costPerResultUsd: 0.0012,
  ...
  reactions: {
    key: "reactions", label: "Reactions given", actorId: "apimaestro~linkedin-profile-reactions", costPerResultUsd: 0.005,
...
export function selectedActors(keys: string[], config: ActorRuntimeConfig): ActorDefinition[] {
  return keys.map((key) => {
    ...
    const actorId = config.actorIds?.[base.key] ?? base.actorId;
